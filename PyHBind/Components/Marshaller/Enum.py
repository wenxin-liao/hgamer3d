...
# Base/Marshaller/Enum.py

import os, re, sys
import PyHBind.Base


class EnumMarshalledType(PyHBind.Base.MarshalledType):

	""" 
	The enum type is pretty similar to the simple C-Type, they can come
	...
	# an Enum can come in the same way as a plain C value, plain, pointer or reference
	#
	...
	def ctype(self, flags, fReturn, basetype):
		...
		"""
		text = "enum Enum" + self.config.nameSuffix
...
#			text = "const " + text
	...
	def fromCpptoC(self, cppname, flags, fReturn, basetype):
		...
		can be a simple cast or some more complex invocations
		"""
		text = "(enum Enum" + self.config.nameSuffix + ") " + cppname
		return text
		
	def chsFuncDefinition(self, flags, fReturn, basetype):
		if self.getOutFlag(flags, fReturn, basetype):
			return "alloca-" + " `" + "Enum" + self.config.nameSuffix + "' " + "peekEnumUtil*"	
		else:
			return "cIntFromEnum" + " `" + "Enum" + self.config.nameSuffix + "' "
